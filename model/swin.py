...
class Audio_SwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, patch_stride = (4, 2),in_chans=3, num_classes=527,
        ...
        super(Audio_SwinTransformer, self).__init__()
        ...
        if self.config.swin_hier_output:
        ...
        if self.config.enable_tscam:
            if self.config.swin_hier_output:
                ...
                for i in range(len(self.depths)):
                    ...
                    SF = self.img_size // zoom_ratio // self.patch_stride[0] // self.freq_ratio
                ...
                SF = self.img_size // (2 ** (len(self.depths) - 1)) // self.patch_stride[0] // self.freq_ratio
            ...
            # self.tscam_norm = nn.Linear(self.num_features, self.num_features)
            # self.tscam_conv = nn.Conv2d(
            #     in_channels= self.num_features,
            #     out_channels= self.num_classes,
            #     kernel_size=3,
    ...
    def forward_features(self, x): 
        # x = self.input_wav2img(x)
        # pad_x = torch.zeros(x.shape[0],x.shape[1],1024 - x.shape[2], x.shape[3]).to(x.device)
        # x = torch.cat((x, pad_x),dim = 2) 
        # # print(x.shape)
        # x = x.permute(0,1,3,2).contiguous() # (b, c, f, t)
        # # print(x.shape)
        # x = x.reshape(x.shape[0], x.shape[1], x.shape[2], 4, x.shape[3] // 4)
        # # print(x.shape)
        ...
        if self.config.swin_hier_output:
            ...
            for i in range(len(hier_x)):
                ...
                fphx = interpolate(hx.permute(0,2,1).contiguous(), self.img_size * self.freq_ratio // hx.shape[2])
        ...
        if self.config.enable_tscam:
            ...
            if self.config.swin_attn_heatmap:
                # for attn
            ...
            # TODO: add avg pool 1d / max pool 1d
            if self.config.swin_use_max:
                x1 = self.a_maxpool(x)
                x2 = self.a_avgpool(x)
                x = x1 + x2
    ...
    def reshape_wav2img(self, x):
        B, C, T, F = x.shape
        target_T = int(self.img_size * self.freq_ratio)
        target_F = self.img_size // self.freq_ratio
    ...
    def repeat_wat2img(self, x, cur_pos):
        ...
        if F < target_F:
            x = nn.functional.interpolate(x, (x.shape[2], target_F), mode="bicubic", align_corners=True)  
        x = x.permute(0,1,3,2).contiguous() # B C F T
        x = x[:,:,:,cur_pos:cur_pos + self.img_size]
    ...
    def forward(self, x: torch.Tensor, mixup_lambda = None, infer_mode = False):# out_feat_keys: List[str] = None):
        ...
        if infer_mode:
            ...
            if self.training:
                cur_pos = random.randint(0, (self.freq_ratio - 1) * self.img_size - 1)
                x = self.repeat_wat2img(x, cur_pos)
                output_dict = self.forward_features(x)
            else:
                output_dicts = []
                for cur_pos in range(0, (self.freq_ratio - 1) * self.img_size + 1, self.img_size):
                    tx = x.clone()
            ...
            if x.shape[2] > self.freq_ratio * self.img_size:
                print("!!larger!!")
                if self.training:
                    x = self.crop_wav(x, crop_size=self.freq_ratio * self.img_size)
                    x = self.reshape_wav2img(x)
                    output_dict = self.forward_features(x)
                else:
